...
<nav class="breadcrumb">
    ...
    <span class="breadcrumb-item active"><i class="fa fa-jpy ico-space" aria-hidden="true"></i>财务</span>
</nav>
    @if(isset($records) && count($records))
        @foreach($records as $record)
            <div class="card card-list">
                ...
                <div class="card-body">
                    <p class="thin-p"><span class="badge badge-{{ $record->add ? 'success' : 'danger' }}">{{ $record->from->name }} {{ $record->add ? '充值' : '消费' }} ¥{{ $record->pay }}</span> 
